...
2. Retrieve the cookie API key generated by the container (first start creates it):
   ```bash
   cat ./data/api_key
   # or: docker exec familylink-auth cat /share/familylink/api_key
   ```
3. Go to **Settings > Devices & Services > Add Integration > Family Link**
4. In the menu, pick **"Manual URL configuration (Docker standalone)"**
5. Enter the auth server URL **including the key**: `http://<your-docker-host>:8099?api_key=<key>`
6. The integration will connect to the standalone container and retrieve authentication cookies

> 🔑 **Why the key?** The `/api/cookies` endpoint hands out your full Google session. The API key (required since v1.7.0) prevents other devices on your network — including your child's — from fetching it. You can also set your own key via the `API_KEY` environment variable.
