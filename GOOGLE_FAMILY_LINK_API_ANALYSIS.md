...
| **Set daily limit duration** | POST | `/kidsmanagement/v1/people/{childId}/timeLimitOverrides:batchCreate` | `[null, childId, [[null, null, 8, deviceId, null, null, null, null, null, null, null, [2, minutes, day_code]]], [1]]` | Type 8 = set daily limit duration for the weekday named by `day_code` (see Day Codes table below): today's code changes today's quota, another day's code sets that weekday's quota, which is what the app's weekly limits screen sends. The rows come back in the `timeLimit` override block as `[uuid, createdMs, 8, deviceId, ..., [2, minutes, day_code]]` and the most recent row per day wins over the weekly value of `data[1]` |
