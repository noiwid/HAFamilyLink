...
| **Set daily limit duration** | POST | `/kidsmanagement/v1/people/{childId}/timeLimitOverrides:batchCreate` | `[null, childId, [[null, null, 8, deviceId, null, null, null, null, null, null, null, [2, minutes, day_code]]], [1]]` | Type 8 = set today's daily limit. **`day_code` MUST be the current day's**: Google applies only the most recent type-8 row, and only when its code is today's. Verified live 2026-09-03: an override posted for Saturday on a Thursday is ignored AND cancels the Thursday override posted before it (applied limit falls back to the weekly row). The rows stay in the `timeLimit` override block as `[uuid, createdMs, 8, deviceId, "", null, null, null, parentId, null, null, [2, minutes, day_code]]`. The app's "weekly limits" screen does NOT use this: it rewrites the weekly rows of `data[1]` (`[code, day, stateFlag, minutes, createdMs, updatedMs]`, `updatedMs` moves), presumably through `timeLimit:update`; that write has not been captured yet |
| **Set bedtime — TODAY only** | POST | `/kidsmanagement/v1/people/{childId}/timeLimitOverrides:batchCreate` | `[null, childId, [[null, null, 9, null, null, null, null, null, null, null, null, null, [2, [startH, startM], [endH, endM], day_code]]], [1]]` | Type 9 = per-day bedtime **override** ("tonight only"). Format: `[status, [startHour, startMin], [endHour, endMin], day_code]`. Status 2=enabled. ⚠️ This does **NOT** change the recurring weekly schedule (next row) — it is the "Ce soir seulement" exception only. `set_bedtime` uses this when `scope=today` (issue #129). |
| **Set daily limit — WEEKLY quota of a weekday** | PUT | `/kidsmanagement/v1/people/{childId}/timeLimit:update?$httpMethod=PUT` | `[null, childId, [null, [[2, null, null, [[slot_id, minutes]]]]], null, [1]]` | Captured on the web app 2026-09-03 ("Programmation de la semaine" of the daily limit): the daily-limit block sits at index `[2][1]`, `[[stateFlag, null, null, [[slot_id, minutes]]]]`, only the modified day is sent and Google merges it into the weekly rows of `data[1]` (`updatedMs` moves). `slot_id` is the day's weekly slot (`CAEQxx` on most accounts, account-specific on some, #157). Response = the full timeLimit object. Does not touch today's applied quota while a type-8 override for today is in force |
| **Set bedtime — WEEKLY schedule** | PUT | `/kidsmanagement/v1/people/{childId}/timeLimit:update?$httpMethod=PUT` | `[null, childId, [[null, null, null, [[day_code, [startH, startM], [endH, endM]]]], null, null, null, []], null, [1]]` | Edits the recurring **weekly** schedule ("Programmation de la semaine" / "Weekly schedule"). Bedtime block at index `[2]`; slot list at `[2][3]`; each slot `["CAEQxx", [startH, startM], [endH, endM]]` (code + start + end, no stateFlag/ruleId). **Only the modified day is sent — Google MERGES it into the existing schedule** (other days untouched), confirmed on-device. `set_bedtime` uses this by default (`scope=weekly`, issue #129). Captured via browser XHR on the real account. |
...
| **Read contact restriction** | GET | `/kidsmanagement/v1/people/{childId}/trustedcontacts` | No body | Response (captured live 2026-08-26): `[[null, "<ts>"], <contacts or null>, <level>, "<country>", "<n>"]`. Level at index `[2]`; `0` on an account where the setting was never touched (behaves like `1`). |
