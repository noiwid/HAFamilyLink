...
### Fixed
- **`set_daily_limit` now resolves the account's own daily-limit slot id and verifies the result (issue #157)** - The override posted by `set_daily_limit` references today's slot of the weekly daily-limit schedule. The client used the static `CAEQxx` code for that slot, which is right on most accounts but not on all of them: on @jar87's second child Google answered HTTP 200 and silently ignored the override, so the app, the device and the sensors kept the old value while the action reported success. The slot id is now read from the account's live `timeLimit` response (static code as fallback, sharing the cached fetch already used for bedtime slots), the batchCreate response is logged at debug level, and the effective value is read back from `appliedTimeLimits` after 3 s and 8 s, as for time bonuses since #141. When Google accepts but does not apply, the action now fails with a clear error instead of a silent log line. The `appliedTimeLimits` parser also prefers the effective daily-limit row of the device block over a recurring or historical copy listed further down, which could overwrite a fresh override with the base value. Diagnosis and patch by @jar87.
