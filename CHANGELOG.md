...
### Added
- **Weekly limits entities: the quota and the bedtime of each weekday, per child** - `number.<child>_<weekday>_limit` (seven per child) shows the screen time quota in force for that weekday, the day's override when there is one and the weekly value otherwise, and setting it posts the quota for that weekday on every device of the child, which is what the weekly limits screen of the app does. `time.<child>_<weekday>_bedtime_start` and `_bedtime_end` (fourteen per child) read and rewrite the weekly bedtime slot of that weekday. The `set_daily_limit` action gets an optional `day` field (1 = Monday, 7 = Sunday), accepted for today only until the weekly write is captured. The weekly rows of `timeLimit` are what the app's weekly screen shows and are the value of each entity; the type-8 override rows are parsed too, and today's entity shows the override Google applies (the most recent one, when it carries today's code). Setting today's entity posts today's override; the other weekdays are read-only for now, because Google ignores an override posted for another day and, worse, it cancels today's (verified live). With strict mode, the values rule keeps the seven weekly quotas as reference and enforces today's applied minutes against today's reference.
