...
| `number.<child>_<weekday>_limit` | Child | Screen time quota of that weekday in minutes, as the weekly limits screen of the app shows it (the weekly value, or today's override when Google applies one; see the `source` and `applied_override` attributes). Setting today's entity posts today's override on every device of the child. The other weekdays are read-only for now: Google only applies an override for the current day, and the weekly write of the app has not been captured yet |
...
| Bedtime hours and the daily limit cannot be changed from Family Link | The weekly bedtime hours (seven days) and the daily quota are put back to the values chosen in Home Assistant (`set_bedtime`, `set_daily_limit`, the `number` and `time` entities). Hours are rewritten in the weekly schedule. For the quota, the reference is the weekly value of each weekday when strict mode starts (or what Home Assistant set since) and the minutes applied today are compared with today's reference: a change made on the Google side shows in the app's weekly screen but is not applied, today's override puts the reference back |
