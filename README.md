...
| Bedtime hours and the daily limit cannot be changed from Family Link | The weekly bedtime hours (all seven days) and each device's daily limit minutes are put back to the values chosen in Home Assistant (`set_bedtime`, `set_daily_limit`). Hours are rewritten in the weekly schedule; minutes are put back as today's override, so a weekly value changed on the Google side is corrected again each morning |
