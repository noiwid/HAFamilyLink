...
| Bedtime hours and the daily limit cannot be changed from Family Link | The weekly bedtime hours (seven days) and the daily quota are put back to the values chosen in Home Assistant (`set_bedtime`, `set_daily_limit`, the `number` and `time` entities). Hours are rewritten in the weekly schedule. For the quota, the reference is the weekly value of each weekday when strict mode starts (or what Home Assistant set since): a weekday changed in the app's weekly screen is written back, and the minutes applied today are put back with today's override |
