...
| Bedtime locked | Bedtime switched on or off on the Google side is put back to the state chosen in Home Assistant |
| Daily limit locked | Same for the daily limit |
| School time locked | Same for school time. Nothing is forced on: if you keep Google's school time off and drive school hours from Home Assistant, it simply stays off |
| Bedtime hours and weekly quotas locked | The weekly bedtime hours (seven days) and the daily quota are put back to the values chosen in Home Assistant (`set_bedtime`, `set_daily_limit`, the `number` and `time` entities). Hours are rewritten in the weekly schedule. For the quota, the reference is the weekly value of each weekday when strict mode starts (or what Home Assistant set since): a weekday changed in the app's weekly screen is written back, and the minutes applied today are put back with today's override |
